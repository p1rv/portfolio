...
const arrayDestructuring = `const countState = useState(0);
...
export const ReactState: React.FC = () => {
  return (
    <div className="flex flex-col items-justify w-full">
      ...
      <pre
        className="font-mono [&>span]:font-mono bg-code-background/80 p-6 rounded-xl text-[14px] font-light [&>span]:font-light [&>span]:text-[14px] mb-6"
        dangerouslySetInnerHTML={{ __html: purifier.sanitize(highlight(codeBasic, languages.js, "tsx")) }}
      />
      <p>
        useState hook returns an array, where on the zero index is the variable itself, and on first it's setter. If you haven't encountered
        array destructuring yet, it's the same as writing
      </p>
      <pre
        ...
        dangerouslySetInnerHTML={{ __html: purifier.sanitize(highlight(arrayDestructuring, languages.js, "tsx")) }}
      />
      <p className="mb-6">
        however destructuring is used almost exclusively. State should be updated ONLY by calling setters with new values - by doing
        otherwise you won't utilize state properly. Everytime state updates, component and it's children render again with new values, but
        by omitting a state setter call, you interfere with React's ability to detect those changes.
      </p>
      <p>
      ...
      <p>
        ...
        don't refer to state variable itself, but to the new value itself. There is also another way (useEffect - more on that later), but
        you should keep your components as concise and simple as you can, and introducing useEffect shouldn't be necessary in mentioned
        case.
      </p>
      <p className="mb-6">
        ...
        value is stale. It was an easiest way to explain the concept though]
      </p>
      <p className="mb-6">
        ...
        styles thrown in, so don't click the button too fast or the screen will be flashing):
      </p>
      <pre
        className="font-mono [&>span]:font-mono bg-code-background/80 p-6 rounded-xl text-[14px] font-light [&>span]:font-light [&>span]:text-[14px]"
